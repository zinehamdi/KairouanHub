        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-8">
                <form action="{{ route('admin.providers.store') }}" method="POST"
                    x-data="{ selectedServices: [], showFloatingButton: false }"
                    @scroll.window="showFloatingButton = window.scrollY > 200">
                    @csrf
                        ...
                        @foreach($services as $categoryName => $categoryServices)
                            <div class="mb-6">
                                <h4 class="font-bold text-brand-dark mb-3 text-lg flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-normal text-gray-500">({{ $categoryServices->count() }}
                                        services)</span>
                                </h4>
                                    ...
                                    @foreach($categoryServices as $service)
                                        <label
                                            ...
                                            <input type="checkbox" name="services[]" value="{{ $service->id }}"
                                                x-model="selectedServices" class="mt-1 h-5 w-5 rounded focus:ring-[#E07A5F]"
                                            ...
                                            <svg x-show="selectedServices.includes({{ $service->id }})"
                                                class="absolute top-2 right-2 w-5 h-5 text-[#E07A5F]" fill="currentColor"
                                                viewBox="0 0 20 20">
                    ...
                    <div x-show="showFloatingButton" x-transition:enter="transition ease-out duration-300"
