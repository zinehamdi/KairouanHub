<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl">{{ __('services.edit_title') }}</h2></x-slot>
    <div class="max-w-3xl mx-auto py-6">
        <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="slug" value="{{ old('slug', $service->slug) }}" class="w-full border rounded px-3 py-2" required>
            ...
            <div>
                ...
                <textarea name="summary" class="w-full border rounded px-3 py-2">{{ old('summary', $service->summary) }}</textarea>
            </div>
            <label class="inline-flex items-center"><input type="checkbox" name="is_active" value="1" {{ $service->is_active ? 'checked' : '' }} class="mr-2"> {{ __('services.active') }}</label>
            <div class="pt-4">
