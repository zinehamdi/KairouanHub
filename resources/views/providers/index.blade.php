    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center mb-8">
            <h1 class="text-4xl md:text-5xl font-heading font-bold text-brand-dark mb-4">
                مقدمو الخدمات في القيروان
            </h1>
            <p class="text-lg md:text-xl text-brand-dark/80 max-w-2xl mx-auto">
                تواصل مع حرفيين محترفين وموثوقين - سباكة، كهرباء، تكييف، نقل أثاث
            </p>
        </div>

        <!-- Advanced Search and Filter -->
        <div class="max-w-5xl mx-auto">
            <form method="GET" action="{{ route('providers.index') }}" class="bg-white rounded-2xl shadow-2xl p-6 border-2 border-accent-DEFAULT/20">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <input 
                            ...
                            class="w-full px-4 py-3 rounded-lg border-2 border-accent-DEFAULT/20 focus:border-accent-DEFAULT focus:ring-4 focus:ring-accent-DEFAULT/20 text-brand-dark font-medium transition-all duration-300"
                        />
                    </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <a href="{{ route('providers.index') }}" class="px-6 py-3 bg-white border-2 border-accent-DEFAULT/30 text-brand-dark font-bold rounded-lg hover:bg-accent-DEFAULT/10 transition-all duration-300">
                        مسح الفلاتر
                    </a>
                    @endif
                    <button type="submit" class="px-8 py-3 bg-gradient-to-r from-accent-DEFAULT to-accent-amber text-white font-bold rounded-lg hover:shadow-gold transition-all duration-300 hover:scale-105 shadow-lg">
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($providers->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                ...
                <article class="group bg-white rounded-2xl shadow-xl hover:shadow-2xl border-2 border-transparent hover:border-accent-DEFAULT/40 transition-all duration-300 hover:-translate-y-2 overflow-hidden">
                    ...
                    <div class="relative h-32 bg-gradient-to-br from-accent-DEFAULT/10 to-accent-amber/10">
                        ...
                        <div class="absolute top-4 right-4">
                            <span class="inline-flex items-center gap-1 px-3 py-1 bg-white/95 backdrop-blur-sm rounded-full border-2 border-accent-DEFAULT/30 shadow-lg font-bold text-sm">
                                @if($provider->badge_level === 'gold')
                                    <span class="text-xl">🥇</span>
                                    <span class="text-accent-amber">ذهبي</span>
                        ...
                        <div class="absolute -bottom-12 left-1/2 -translate-x-1/2">
                            <div class="w-24 h-24 rounded-full bg-gradient-to-br from-accent-DEFAULT to-accent-amber flex items-center justify-center shadow-gold border-4 border-white">
                    ...
                    <div class="pt-16 px-6 pb-6 text-center">
                        ...
                        @if($provider->city)
                        <div class="flex items-center justify-center gap-1 text-brand-dark/70 mb-3">
                        ...
                        <div class="flex items-center justify-center gap-1 mb-4">
                            @for($i = 1; $i <= 5; $i++)
                                ...
                                @else
                                    <svg class="w-5 h-5 text-accent-DEFAULT/30" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                @endif
                            @endfor
                            <span class="ml-2 text-accent-DEFAULT font-bold">{{ number_format($provider->avg_rating, 1) }}</span>
                        </div>
                        @endif

                        <!-- View Profile Button -->
                        <a href="{{ route('providers.show', $provider->id) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-accent-DEFAULT to-accent-amber text-white font-bold rounded-lg shadow-lg hover:shadow-gold hover:scale-105 transition-all duration-300 w-full justify-center">
                            عرض الملف الشخصي
        ...
        @else
            ...
            <div class="text-center py-16">
                ...
                <a href="{{ route('providers.index') }}" class="inline-block px-8 py-4 bg-gradient-to-r from-accent-DEFAULT to-accent-amber text-white font-bold rounded-lg shadow-lg hover:shadow-gold hover:scale-105 transition-all duration-300">
                    عرض الكل
                </a>
            </div>
        @endif
        ...
        <div class="mt-16 text-center">
            <div class="inline-block bg-gradient-to-br from-accent-DEFAULT to-accent-amber p-12 rounded-2xl shadow-2xl max-w-3xl">
                ...
                <a href="{{ route('register') }}" class="inline-block px-10 py-5 bg-white text-accent-DEFAULT font-bold text-xl rounded-lg shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300">
                    انضم الآن
                </a>
            </div>
        </div>
