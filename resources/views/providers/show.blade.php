    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <div class="flex gap-2 mt-2 text-xs">
                @if($profile->badge_level)<span class="px-2 py-0.5 bg-amber-100 text-amber-700 rounded">{{ $profile->badge_level }}</span>@endif
                @if($profile->avg_rating)<span class="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded">★ {{ $profile->avg_rating }}</span>@endif
                <span class="px-2 py-0.5 bg-gray-200 dark:bg-gray-700 rounded">{{ $profile->completed_jobs }} jobs</span>
            </div>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('requests.create') }}" class="self-start px-4 py-2 bg-indigo-600 text-white rounded text-sm">{{ __('providers.show.request_cta') }}</a>
            @auth
                @if(auth()->user()->hasRole('client'))
                    <a href="{{ route('requests.mine') }}" class="self-start px-4 py-2 bg-gray-600 text-white rounded text-sm">{{ __('requests.mine_link') ?: 'My Requests' }}</a>
                @endif
            @endauth
        </div>
    </div>
            ...
            <x-card>
                ...
                <ul class="text-sm space-y-1">
                    @foreach($profile->services as $svc)
                        <li class="flex justify-between"><span>{{ $svc->name }}</span><span class="text-gray-500 text-xs">@if($svc->pivot->price_min || $svc->pivot->price_max) {{ $svc->pivot->price_min }} - {{ $svc->pivot->price_max }} @endif</span></li>
                    @endforeach
                </ul>
            </x-card>
