    ...
    <div class="relative bg-mediterranean-gradient py-20">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-10">
                <h1 class="text-5xl md:text-6xl font-heading font-bold text-brand-dark mb-6">
                ...
                <form method="GET" action="{{ route('services.index') }}" class="bg-white rounded-2xl shadow-soft-lg p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="md:col-span-2">
                            <div class="relative">
                                <div
                                    class="absolute inset-y-0 right-0 rtl:right-0 rtl:left-auto ltr:left-auto ltr:right-0 pr-4 rtl:pr-4 rtl:pl-0 flex items-center pointer-events-none">
                                    <svg class="h-6 w-6 text-accent-DEFAULT" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                ...
                                <input type="text" name="q" value="{{ request('q') }}"
                                    ...
                                    class="input-mediterranean pr-12 rtl:pr-12 rtl:pl-4" />
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            <label
                                class="flex items-center gap-3 px-6 py-4 bg-kairouan-warm-cream rounded-xl cursor-pointer hover:bg-gray-100 transition-all duration-300 w-full">
                                <input type="checkbox" name="active" value="1" @checked(request()->boolean('active', true))
                                    class="w-5 h-5 text-accent-DEFAULT rounded focus:ring-mediterranean-blue focus:ring-2" />
                                <span class="font-bold text-brand-dark">{{ __('services.active_only') }} ✓</span>
                            </label>
                        </div>
                    </div>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($services->total() > 0)
                <div class="text-center mb-12">
                    <div
                        class="inline-block px-8 py-4 bg-gradient-to-r from-accent-DEFAULT to-accent-amber text-white font-bold text-lg rounded-full shadow-soft">
                        🎯 {{ __('services.services_available', ['count' => $services->total()]) }}
                    </div>
                </div>
            @endif
                ...
                @forelse($services as $service)
                    <article class="card-mediterranean overflow-hidden">
                        ...
                        <div class="relative h-40 bg-mediterranean-gradient overflow-hidden">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center">
                                <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-soft">
                                    @if(str_contains(strtolower($service->name), 'سباكة') || str_contains(strtolower($service->name), 'plumb'))
                                        <svg class="w-10 h-10 text-accent-DEFAULT" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @elseif(str_contains(strtolower($service->name), 'كهرباء') || str_contains(strtolower($service->name), 'electric'))
                                        <svg class="w-10 h-10 text-accent-DEFAULT" fill="currentColor" viewBox="0 0 20 20">
                                            <path
                                    ...
                                    @elseif(str_contains(strtolower($service->name), 'نقل') || str_contains(strtolower($service->name), 'moving'))
                                        <svg class="w-10 h10 text-accent-DEFAULT" fill="currentColor" viewBox="0 0 20 20">
                                            <path
                            ...
                            <!-- Category Badge -->
                            <div class="absolute top-4 left-4 rtl:left-4 rtl:right-auto">
                        ...
                        <div class="p-6">
                            <h3
                                class="text-xl font-bold text-brand-dark mb-3 hover:text-accent-DEFAULT transition-colors duration-300">
                                {{ e($service->name) }}
                            </h3>
                            ...
                            <a href="{{ route('services.show', $service->slug) }}"
                                ...
                                <svg class="w-5 h-5 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform duration-300"
                ...
                @empty
                    ...
                    <div class="col-span-full text-center py-20">
                        <div class="inline-block p-8 bg-white rounded-2xl shadow-soft">
                            ...
                            <h3 class="text-2xl font-bold text-brand-dark mb-4">{{ __('services.no_services_title') }}</h3>
                            <p class="text-gray-600 text-lg mb-6">{{ __('services.no_services_desc') }}</p>
